...
function (BaseView, loading) {
    ...
    function View(view, params) {
        ...
        view.querySelector('.m3uEditorConfigForm').addEventListener('submit', function (e) {
            e.preventDefault();
            saveConfig(self);
        });
        ...
        view.querySelector('.m3uEditorConfigForm').addEventListener('click', function (e) {
    ...
    View.prototype.onResume = function (options) {
        ...
        var followUp = function () {
            try { loadDashboard(self.view); } catch (e) {
                console.error('M3uEditor: loadDashboard failed in onResume', e);
            }
            try { checkForUpdate(self.view); } catch (e) {
                console.error('M3uEditor: checkForUpdate failed in onResume', e);
            }
        };
    ...
    function loadConfig(instance) {
        ...
        return ApiClient.getPluginConfiguration(pluginId).then(function (config) {
            ...
            try { renderHealthBar(view, config); } catch (e) {
                console.error('M3uEditor: renderHealthBar (early) failed', e);
            }
            ...
            loading.hide();
            console.error('M3uEditor: failed to load plugin configuration', err);
            ...
            try {
                ...
                console.error('M3uEditor: renderHealthBar (fallback) failed', e2);
            }
        });
    }
    ...
    function applyScheduleToTasks(view, config, apiClient) {
        apiClient.ajax({ url: apiClient.getUrl('ScheduledTasks'), type: 'GET' })
            .then(function (tasks) {
                var pluginTasks = tasks.filter(function (t) {
                    return t.Category === 'm3u-editor for Emby';
                });
                var triggers = buildTriggers(config);
                pluginTasks.forEach(function (task) {
                    apiClient.ajax({
    ...
    function browserNavigate(view, path) {
        ...
        var url = ApiClient.getUrl('M3uEditor/BrowsePath');
    ...
    function validateStrmPath(view) {
        ...
        ApiClient.ajax({
            type: 'POST',
            url: ApiClient.getUrl('M3uEditor/ValidateStrmPath'),
            contentType: 'application/json',
    ...
    function testDispatcharrConnection(instance) {
        ...
        ApiClient.ajax({
            type: 'POST',
            url: ApiClient.getUrl('M3uEditor/TestDispatcharr'),
            contentType: 'application/json',
    ...
    function checkProbeDataCoverage(instance) {
        ...
        ApiClient.ajax({
            type: 'GET',
            url: ApiClient.getUrl('M3uEditor/ProbeDataCoverage'),
            dataType: 'json'
        }).then(function (result) {
            if (!result) {
    ...
    function loadDispatcharrProfiles(instance) {
        ...
        var apiUrl = ApiClient.getUrl('M3uEditor/DispatcharrProfiles');
    ...
    function loadCategories(instance) {
        ...
        var apiUrl = ApiClient.getUrl('M3uEditor/Categories/Live');
    ...
    function loadVodCategories(instance) {
        ...
        var apiUrl = ApiClient.getUrl('M3uEditor/Categories/Vod');
    ...
    function loadSeriesCategories(instance) {
        ...
        var apiUrl = ApiClient.getUrl('M3uEditor/Categories/Series');
    ...
    function pollSyncProgress(view, type) {
        ...
        var apiUrl = ApiClient.getUrl('M3uEditor/Sync/Status');
    ...
    function syncMovies(view) {
        ...
        var apiUrl = ApiClient.getUrl('M3uEditor/Sync/Movies');
    ...
    function syncSeries(view) {
        ...
        var apiUrl = ApiClient.getUrl('M3uEditor/Sync/Series');
    ...
    function deleteContent(view, type) {
        ...
        resultEl.querySelector('.deleteConfirmYes').addEventListener('click', function () {
            ...
            ApiClient.ajax({
                type: 'DELETE',
                url: ApiClient.getUrl('M3uEditor/Content/' + type),
                dataType: 'json'
            }).then(function (result) {
                btn.disabled = false;
                setPillResult(resultEl, result.Success, result.Message);
    ...
    function refreshCache(view) {
        ...
        var apiUrl = ApiClient.getUrl('M3uEditor/RefreshCache');
    ...
    function syncGuideMappings(view) {
        ...
        ApiClient.ajax({
            type: 'POST',
            url: ApiClient.getUrl('M3uEditor/SyncGuideMappings'),
            dataType: 'json'
        }).then(function (result) {
            btn.disabled = false;
            setPillResult(resultEl, result.Success, result.Message);
    ...
    function loadDashboard(view) {
        var apiUrl = ApiClient.getUrl('M3uEditor/Dashboard') +
        ...
        ApiClient.getJSON(apiUrl).then(function (data) {
            ...
            var prevVer = localStorage.getItem('m3u-editor-for-emby-plugin-version');
            if (data.PluginVersion && prevVer && data.PluginVersion !== prevVer
                && !sessionStorage.getItem('m3u-editor-for-emby-cache-bust')) {
                sessionStorage.setItem('m3u-editor-for-emby-cache-bust', '1');
                var v = document.documentElement.getAttribute('data-appversion') || '';
                Promise.all([
                    fetch('configurationpage?name=m3ueditorconfig&v=' + v, { cache: 'reload' }),
                    fetch('configurationpage?name=m3ueditorconfigjs&v=' + v, { cache: 'reload' })
                ]).then(function () { location.reload(); });
                return;
            }
            if (data.PluginVersion) localStorage.setItem('m3u-editor-for-emby-plugin-version', data.PluginVersion);
            ...
            console.warn('M3uEditor: dashboard load failed', err);
            ...
            try {
                ...
                var managedStatusEl = view.querySelector('.managedPublishingStatus');
                if (managedStatusEl) {
                ...
                console.error('M3uEditor: dashboard fallback render failed', e);
            }
        });
    ...
    function loadFailedItems(view) {
        ...
        ApiClient.getJSON(ApiClient.getUrl('M3uEditor/Sync/FailedItems')).then(function (items) {
    ...
    function retryFailed(view) {
        ...
        ApiClient.ajax({ type: 'POST', url: ApiClient.getUrl('M3uEditor/Sync/RetryFailed') })
            .then(function (data) {
    ...
    function checkForUpdate(view) {
        ...
        var apiUrl = ApiClient.getUrl('M3uEditor/CheckUpdate');
        ...
        ApiClient.getJSON(apiUrl).then(function (data) {
            ...
            console.error('M3uEditor: update check failed', err);
        });
    }
    ...
    function restartEmby(view) {
        ...
        ApiClient.ajax({
            type: 'POST',
            url: ApiClient.getUrl('M3uEditor/RestartEmby')
        }).then(function () {
    ...
    function renderManagedPublishing(view, managed) {
        ...
        var reconcileBtn = view.querySelector('.btnManagedReconcile');
        var previousBtn = view.querySelector('.btnManagedPreviousPage');
        var nextBtn = view.querySelector('.btnManagedNextPage');
        var pageEl = view.querySelector('.managedPublishingPage');
        ...
        detailsEl.innerHTML =
            ...
            '<div><strong>Owned files:</strong> ' + escapeHtml(managed.TotalFiles || 0) +
                ' (' + escapeHtml(managed.TotalStrmFiles || 0) + ' STRM)</div>' +
        ...
        if (!managed.Mappings) {
            ...
            catch (e) { console.warn('M3uEditor: managed mapping state parse failed', e); }
        }
        ...
        if (jobRunning) {
            ...
            if ((job.State === 'succeeded' || job.State === 'failed') && job.Result) {
                setPillResult(resultEl, job.State === 'succeeded', job.Result.Message || 'Managed action finished.');
            }
        }
    }
    ...
    function reconcileManaged(view) {
        ...
        ApiClient.ajax({
            ...
            setPillResult(resultEl, result.Success,
                result.Accepted ? 'Managed reconcile accepted.' : (result.Message || 'Managed reconcile is running.'));
            loadDashboard(view);
        }).catch(function (err) {
            console.error('M3uEditor: managed reconcile failed', err);
            button.disabled = false;
            setPillResult(resultEl, false, 'Managed reconcile request failed. Click Reconcile Now to retry.');
        });
    }
    ...
    function rollbackManaged(view) {
        ...
        ApiClient.ajax({
            ...
            console.error('M3uEditor: managed rollback failed', err);
            button.disabled = false;
            setPillResult(resultEl, false, 'Managed rollback request failed. Select the mapping and retry.');
        });
    }
    ...
    function dashboardSyncAll(instance) {
        ...
        ApiClient.getPluginConfiguration(pluginId).then(function (config) {
            ...
            var movieUrl = ApiClient.getUrl('M3uEditor/Sync/Movies');
            var seriesUrl = ApiClient.getUrl('M3uEditor/Sync/Series');
            var movieMsg = '';
